<header id="header" class="header--has-logo header--has-title header--has-tagline header--has-search">
    <div class="header__inner">
        <div class="lsvr-container">
            <div class="header__container" style="display: flex;justify-content: space-between;color: #fff;">
                    <a class="" href="<?php echo e(url('/')); ?>">
                        نظام شجرة العائلة 
                    </a>
                    <div>
                        <a href="<?php echo e(route('login')); ?>">تسجيل الدخول</a> 
                    </div>


                <!-- HEADER NAVBAR : begin -->
                <!-- <div id="header-navbar" class="header-navbar">
                    <div class="header-navbar__inner"> -->



                        <!-- PRIMARY HEADER MENU : begin -->
                        <!-- <nav id="header-menu-primary" class="header-menu-primary" data-label-expand-popup="Expand submenu" data-label-collapse-popup="Collapse submenu" aria-label="Header Menu">

                            <ul id="menu-header-menu" class="header-menu-primary__list" role="menu">
                                <li id="header-menu-primary__item-309" class="header-menu-primary__item header-menu-primary__item--level-0 menu-item menu-item-type-post_type menu-item-object-page menu-item-home <?php echo e(request()->routeIs('family.home') ? 'current-menu-ancestor': ''); ?>" role="presentation">

                                    <a href="<?php echo e(route('family.home')); ?>" id="header-menu-primary__item-link-309" class="header-menu-primary__item-link header-menu-primary__item-link--level-0" role="menuitem">

                                        <span class="header-menu-primary__item-link-label">الرئيسيه</span>


                                    </a>


                                </li>


                                

                            </ul>
                            
                        </nav> -->
                        <!-- PRIMARY HEADER MENU : end -->









                    <!-- </div>
                </div> -->
                <!-- HEADER NAVBAR : end -->

                <!-- HEADER NAVBAR TOGGLE : begin -->
                <!-- <button id="header-navbar-toggle" class="header-navbar-toggle" type="button" title="Open menu" data-label-expand-popup="Open menu" data-label-collapse-popup="Close menu" aria-controls="header-navbar" aria-haspopup="true" aria-expanded="false">
                    <i class="la la-bars"></i>
                </button> -->
                <!-- HEADER NAVBAR TOGGLE : end -->


            </div>
        </div>
    </div>
</header><?php /**PATH C:\laragon\www\family-tree\resources\views/frontend/layout/partials/header.blade.php ENDPATH**/ ?>